...
const ResetPasswordPage: React.FC = () => {
    ...
    // If we don't have a session and auth is NOT loading, it means the recovery link failed
    // to establish a session, or it already expired.
    useEffect(() => {
        if (!authLoading && !session && !success) {
            setError('Sessão de recuperação não encontrada ou expirada. Tente solicitar um novo link.');
        }
    }, [authLoading, session, success]);

    const handleReset = async (e: React.FormEvent) => {
        e.preventDefault();

        if (!session) {
            setError('Sessão de autenticação ausente. Verifique se o link no e-mail ainda é válido.');
            return;
        }
                    ...
                    <form onSubmit={handleReset} className="space-y-6">
                        ...
                        {!session && !authLoading && (
                            <button
                                ...
                                className="w-full h-12 text-slate-500 font-bold text-xs uppercase tracking-widest hover:text-primary transition-colors"
                            >
                                Voltar para o Login
                            </button>
                        )}
